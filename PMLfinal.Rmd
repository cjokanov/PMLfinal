...
data<-read.csv(".Ruserdata/pml_train.csv", stringsAsFactors=FALSE )
ftest<-read.csv(".Ruserdata/pml_test.csv",stringsAsFactors=FALSE )
...
```{r}
training<-data.frame(training[,1:10], training[,37:49], training[,60:68], training[,84:86], training[,113:124], training[,151:160])
testing<-data.frame(testing[,1:10], testing[,37:49], testing[,60:68], testing[,84:86], testing[,113:124], testing[,151:160])
ftest<-data.frame(ftest[,1:10], ftest[,37:49], ftest[,60:68], ftest[,84:86], ftest[,113:124], ftest[,151:160])


corM<-cor(training[,7:56])
...
preObj<-preProcess(training[,-37],method="knnImpute")
...
I applied svmLinear model, with corssvalidation (5 sections). 
...
```{r}
predsvm<-predict(mod_svm,testing)
